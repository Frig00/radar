        ...
        when (operation) {
            ...
            is EnableNotifications -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    val payload = when {
                        characteristic.isIndicatable() || characteristic.isNotifiable() -> {
                            Timber.d("Using NOTIFICATION for characteristic: ${characteristic.uuid}") // Log change
                    ...
                    characteristic.getDescriptor(cccdUuid)?.let { cccDescriptor ->
                        Timber.d("Enabling notifications for characteristic: ${characteristic.uuid}") // ADD THIS LOG
                        if (!gatt.setCharacteristicNotification(characteristic, true)) {
                            Timber.e("setCharacteristicNotification failed for ${characteristic.uuid}")
                            signalEndOfOperation()
                            return
                        }
                        Timber.d("setCharacteristicNotification SUCCESS for characteristic: ${characteristic.uuid}") // ADD THIS LOG
    ...
    private val callback = object : BluetoothGattCallback() {
        ...
        override fun onDescriptorWrite(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        Timber.i("Wrote to descriptor $uuid, status: GATT_SUCCESS | operation type: $operationType") // MODIFIED LOG
                        Timber.i("Descriptor UUID: $uuid, Characteristic UUID: ${descriptor.characteristic.uuid}") // ADD THIS LOG - Char UUID
